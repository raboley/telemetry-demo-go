...
func main() {
	...
	tp, err := telemetry.InitTracing("subscriber-api-dapr", "1.0.0")
	if err != nil {
	...
	// Create Dapr repository
	daprRepo := repository.NewDaprSubscriberRepository(daprClient, "statestore")

	config := &app.Config{
		ServiceName:    "subscriber-api-dapr",
